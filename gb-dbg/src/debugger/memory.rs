...
pub struct MemoryEditorBuilder<T: DebugRW> {
    address_ranges: Vec<(String, Range<usize>)>,
    memory: T,
}

impl<T: DebugRW> MemoryEditorBuilder<T> {
    ...
    pub fn build(self) -> GBMemoryEditor<T> {
        ...
        let mut mem_edit = MemoryEditor::new(|mem, address| <T as DebugRW>::read(mem, address))
        ...
        GBMemoryEditor {
            ...
            memory_editor: mem_edit.with_write_function(|mut mem, address, value| {
                <T as DebugRW>::write(&mut mem, address, value)
            }),
        }
    }
}
