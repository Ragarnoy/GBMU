...
const SETTINGS_FORMAT: &str = "json";
...
pub fn load() -> Option<gb_joypad::Config> {
    ...
    match OpenOptions::new().read(true).open(&settings_path) {
        Ok(mut file) => match file.read_to_string(&mut content) {
            Ok(_) => match serde_json::from_str::<gb_joypad::Config>(&content) {
...
pub fn save(conf: gb_joypad::Config) {
    ...
    match std::fs::File::create(&settings_path) {
        Ok(mut setting_file) => {
            let conf_value = serde_json::json!(conf);
            if let Err(err) = setting_file.write_all(conf_value.to_string().as_bytes()) {
                log::error!(
